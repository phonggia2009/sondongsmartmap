...
import { getRelicColor, getRelicBgColor, getRelicEmoji } from '@/utils/relicUtils';
import type { Relic, RelicType } from '@/types';
import { Award, MapPin, Calendar, Landmark, Navigation } from 'lucide-react';
      ...
      {clusters.map(cluster => {
        ...
        return (
          <Marker
            ...
            ref={ref => {
              if (ref) {
                markerRefs.current.set(relic.id, ref);
                if (isSelected) {
                  setTimeout(() => {
                    try {
                      ref.openPopup();
                ...
                markerRefs.current.delete(relic.id);
              }
            }}
            icon={L.divIcon({
              html: markerHtml,
            ...
            eventHandlers={{
              ...
              mouseover: () => setHoveredId(relic.id),
